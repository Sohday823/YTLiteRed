...
- (void)updateYTLiteSectionWithEntry:(id)entry {
    ...
    // Downloading section with button placement
    ...
    selectBlock:^BOOL (YTSettingsCell *cell, NSUInteger arg1) {
        ...
        accessibilityIdentifier:@"YTLiteSectionItem"
        detailTextBlock:^NSString *() {
            NSArray *positionLabels = @[LOC(@"UnderPlayer"), LOC(@"Overlay"), LOC(@"Both"), LOC(@"NewButton")];
        ...
        selectBlock:^BOOL (YTSettingsCell *cell, NSUInteger arg1) {
            ...
            YTSettingsPickerViewController *picker = [[%c(YTSettingsPickerViewController) alloc] initWithNavTitle:LOC(@"YtlButtonPosition") pickerSectionTitle:nil rows:positionRows selectedItemIndex:ytlInt(@"buttonPositionIndex") parentResponder:[self parentResponder]];
            [settingsViewController pushViewController:picker];
            return YES;
        }];

        [rows addObject:buttonPosition];
    ...
    if (ytlBool(@"advancedMode")) {
        ...
        selectBlock:^BOOL (YTSettingsCell *cell, NSUInteger arg1) {
            ...
            // Speed Location setting (only shown when SpeedByLongTap is enabled)
